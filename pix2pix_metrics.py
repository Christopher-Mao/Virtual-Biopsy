...
def calculate_metrics(generated, target):
    generated = generated.permute(0, 2, 3, 1).cpu().numpy()  # (N, H, W, C)
    target = target.permute(0, 2, 3, 1).cpu().numpy()        # (N, H, W, C)
    ...
    for i in range(batch_size):
        ...
        gen_img = (gen_img * 255).astype(np.uint8)  # Convert to [0, 255] for SSIM/PSNR
        ...
        win_size = max(win_size, 3)  # Ensure win_size is at least 3 (SSIM minimum)

        # Calculate MSE
        mse_value = np.mean((gen_img - tgt_img) ** 2)
        mse_list.append(mse_value)

        # Calculate SSIM
        try:
            ssim_value = ssim(gen_img, tgt_img, win_size=win_size, channel_axis=-1)
        except ValueError:
            ssim_value = 0  # Assign a default value if SSIM fails
        ssim_list.append(ssim_value)

        # Calculate PSNR
        psnr_value = psnr(tgt_img, gen_img, data_range=255)
...
def evaluate_model(generator, test_loader, device):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # Calculate metrics
            mse_score, ssim_score, psnr_score = calculate_metrics(generated_images, target_images)
            mse_scores.append(mse_score)
            ssim_scores.append(ssim_score)
            psnr_scores.append(psnr_score)

    print(f"Average MSE: {np.mean(mse_scores):.4f}, SSIM: {np.mean(ssim_scores):.4f}, PSNR: {np.mean(psnr_scores):.4f}")
    return np.mean(mse_scores), np.mean(ssim_scores), np.mean(psnr_scores)
...
#resume_from_checkpoint = "/content/drive/MyDrive/Colab Notebooks/Computer Vision/Shared Folder/Computer Vision/checkpoints/checkpoint_epoch_85.pth"
resume_from_checkpoint = None
start_epoch = 0
if resume_from_checkpoint:
    start_epoch = load_checkpoint(generator, discriminator, generator_optimizer, discriminator_optimizer, resume_from_checkpoint)
# Training loop
for epoch in range(start_epoch, num_epochs):
    ...
    mse_value, ssim_value, psnr_value = evaluate_model(generator, test_loader, device)
    mse_list.append(mse_value)
    ssim_list.append(ssim_value)
    psnr_list.append(psnr_value)
    ...
    with open(os.path.join(metrics_path, "training_metrics.json"), "w") as f:
        json.dump(metrics, f)
...
def plot_metrics(metrics_path, output_dir="plots"):
    ...
    # Load metrics
    with open(os.path.join(metrics_path, "training_metrics.json"), "r") as f:
        metrics = json.load(f)

    # Plot SSIM
    plt.figure(figsize=(10, 6))
    plt.plot(metrics["ssim"], label="SSIM", marker='o')
    plt.xlabel("Epoch")
    plt.ylabel("SSIM")
    ...
    # Plot PSNR
    plt.figure(figsize=(10, 6))
    plt.plot(metrics["psnr"], label="PSNR", marker='o', color='orange')
    plt.xlabel("Epoch")
    plt.ylabel("PSNR (dB)")
    ...
    # Plot MSE
    plt.figure(figsize=(10, 6))
    plt.plot(metrics["mse"], label="MSE", marker='o', color='red')
    plt.xlabel("Epoch")
    plt.ylabel("MSE")
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(metrics["gen_loss"], label="Generator Loss", marker='o', color='blue')
    plt.plot(metrics["disc_loss"], label="Discriminator Loss", marker='o', color='green')
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
